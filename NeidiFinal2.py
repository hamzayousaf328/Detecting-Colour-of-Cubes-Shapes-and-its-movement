...
path_to_video = "wooden_block4.mp4"

contours = ShapeFunction(path_to_video)
...
print("THIS IS BOUNDING RECT: ", boundRect)
...
for i in range(0, 5):   
    trackersForBlue.append(cv2.TrackerCSRT_create())
...
for i in range(0, 2):
    bboxRed.append([boundRect[i][0], boundRect[i][1], boundRect[i][2], boundRect[i][3]])
    trackersForRed[i].init(img, bboxRed[i])

for i in range(2, 5):
    print("III: ", i)
    bboxYellow.append([boundRect[i][0], boundRect[i][1], boundRect[i][2], boundRect[i][3]])

for i in range(5, 10):
    bbox.append([boundRect[i][0], boundRect[i][1], boundRect[i][2], boundRect[i][3]])
...
for i in range(0, 5):
    trackersForBlue[i].init(img, bbox[i])
...
def drawBox(img, bbox, bboxRed, bboxYellow):
    ...
    for i in range(0, len(bboxYellow)):
        ...
        cv2.putText(img, ""+str(i)+"-"+str(xYellow[i])+","+str(yYellow[i]), (xYellow[i], int(yYellow[i]+(hYellow[i]/2))), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2) 
    ...
    for i in range(0, len(bboxRed)):
        ...
        cv2.putText(img, ""+str(i)+"-"+str(xRed[i])+","+str(yRed[i]), (xRed[i], int(yRed[i]+(hRed[i]/2))), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 0), 2)
    ...
    for i in range(0, len(bbox)):
        ...
        cv2.putText(img, ""+str(i)+"-"+str(x[i])+","+str(y[i]), (x[i], int(y[i]+(h[i])/2)), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 0), 2)
        # checkLength()
        try:
            bottomXValue[i] = x[i]
        except:
...
while True:
    ...
    for i in range(0, len(bbox)):
        bbox[i] = trackersForBlue[i].update(img)
